package com.yanhao.picReader.utils;
...
public class FileUtil {
    public static String getFileModifiedTime(File file){
        ...
        String fileTime=c.getTime().toLocaleString();
        return fileTime;
    }
    /**
     * 计算压缩比例
     ...
     * @param file
     *            文件
     * @return 根据图片的后缀名判断("jpg","png","svg","jpeg","bmp")
     */
    public static boolean isPic(File file) {
        String[] types = { "jpg", "png", "svg", "jpeg", "bmp" };
        boolean flag = false;
        for (String type : types) {
            flag = flag || isCertainType(file, type);
            if (flag) {
    ...
    public static List<File> getPicFiles(File rootFile){
        List<File>mediaFiles=FileUtil.listFiles(rootFile);
        List<File> delFiles=new ArrayList<File>();
        for(File f:mediaFiles){
            if(!FileUtil.isPic(f)){
